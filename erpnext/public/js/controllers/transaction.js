...
erpnext.TransactionController = class TransactionController extends (
	...
	setup() {
		...
		frappe.ui.form.on(
			...
			function (frm, cdt, cdn) {
				...
				var has_margin_field = frappe.meta.has_field(
					cdt,
					"margin_type"
				);
		...
		if (
			this.frm.fields_dict["items"].grid.get_field(
	...
	setup_quality_inspection() {
		...
		quality_inspection_field.get_route_options_for_new_doc = function (
			row
		) {
	...
	is_a_mapped_document(item) {
		...
		return (
			mappped_fields.map((field) => item[field]).filter(Boolean).length >
			0
		);
	}
	...
	validate_company_and_party() {
		...
		$.each(["company", "customer"], function (i, fieldname) {
			if (
				...
				!["Purchase Order", "Purchase Invoice"].includes(
					me.frm.doc.doctype
				)
			) {
	...
	get_terms() {
		...
		erpnext.utils.get_terms(
			this.frm.doc.tc_name,
			this.frm.doc,
			function (r) {
				if (!r.exc) {
						...
						$.each(me.frm.doc.items || [], function (i, item) {
							if (
								...
								item.item_tax_template =
									r.message[item.name].item_tax_template;
								item.item_tax_rate =
									r.message[item.name].item_tax_rate;
								me.add_taxes_from_item_tax_template(
									item.item_tax_rate
								);
							}
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message && !r.exc) {
						...
						me.update_payment_schedule_grid_labels(
							company_currency
						);
					}
				},
